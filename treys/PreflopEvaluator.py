...
class unknownEvaluator:
    ...
    def evaluate(self, hand: list[int], players: int, board = False) -> int:
        """
        ...
        if board:
            ...
            known_cards = hand
        return self.hand_size_map[len(known_cards)](known_cards)

    def _two(self,known_cards: Sequence[int]) -> int:
        """
        ...
        Combinations of each iteration: Pair=6, s=4, o=12
        """
        # if paired > 8
        if (known_cards[0] & known_cards[1]) >> 16 and int(known_cards[0] >> 16) > 17 :
            return

        elif known_cards[0] & known_cards[1] & 0xF000:
            suited_rank = Card.prime_product_from_hand(known_cards)

    def _five(self, known_cards: Sequence[int]) -> int:
        """
        ...
        """
        # if flush
        if known_cards[0] & known_cards[1] & known_cards[2] & known_cards[3] & known_cards[4] & 0xF000:
            handOR = (known_cards[0] | known_cards[1] | known_cards[2] | known_cards[3] | known_cards[4]) >> 16
    ...
    def _six(self, known_cards: Sequence[int]) -> int:
        """
        ...
        for combo in itertools.combinations(known_cards, 5):
    ...
    def _seven(self, known_cards: Sequence[int]) -> int:
        """
